...
def count_zero_coefficients(coeff):
    if isinstance(coeff, np.ndarray):
        coeff = coeff.flatten()
    return str(list(coeff).count(0.0)) + "/" + str(len(coeff))
...
def run_xgboost_regression(df, X, y, param, dataset, reg_alpha, reg_lambda, metric, density):
    ...
    xgb_zero = count_zero_coefficients(xgb_get_weights(bst))
    xgb_iterations = bst.best_iteration + early_stopping_rounds
    df.loc[len(df)] = [dataset, X.shape[1], X.shape[0], density, 'Regression', reg_alpha, reg_lambda,
                       'XGBoost (' + param['updater'] + ')', metric,
                       xgb_score,
...
def run_sklearn_classification(df, X, y, algo, dataset, reg_alpha, reg_lambda, metric, density, num_class):
    tmp = time.time()
    C = None
    penalty = None
    ...
    sklearn_zero = count_zero_coefficients(sklearn.coef_)
    df.loc[len(df)] = [dataset, X.shape[1], X.shape[0], density, 'Classification', reg_alpha, reg_lambda,
                       'Sklearn ({})'.format(algo),
...
def run_classification(df, dataset, X, y, reg_alpha, reg_lambda, standardize=True, solvers=None):
    ...
    if 'Sklearn (liblinear)' in solvers:
        run_sklearn_classification(df, X, y, 'liblinear', dataset, reg_alpha, reg_lambda, metric, density, num_class)
    if 'Sklearn (sag)' in solvers:
        run_sklearn_classification(df, X, y, 'sag', dataset, reg_alpha, reg_lambda, metric, density, num_class)
    if 'Sklearn (lbfgs)' in solvers:
        run_sklearn_classification(df, X, y, 'lbfgs', dataset, reg_alpha, reg_lambda, metric, density, num_class)
...
X, y = datasets.get_cover_type(num_rows)
solvers = ['XGBoost', 'Sklearn (sag)', 'Sklearn (lbfgs)', 'Glmnet']
run_classification(df, 'Cover type', X, y, 0.0, 0.1, True, solvers)
